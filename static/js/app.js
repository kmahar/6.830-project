(function() {
	...
	d3.json('static/json/us-states.json', function(error, topology) {
	    g.selectAll('path')
	...
	function displayData(data, emotion) {
		...
		getUserInfo(data, function(user){
			document.querySelector('.emotion').style.backgroundImage = 'url(static/images/'+ emotion.icon +')';
			...
			// faceIcon.enter()
			// 	.append('svg:image')
			// 	.attr('xlink:href', 'static/images/'+ emotion.icon)
